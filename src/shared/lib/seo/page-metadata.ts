...
/**
 * Страница пагинации: из индекса убираем, обход по ссылкам оставляем.
 *
 * `follow` здесь принципиален. Карточки на второй и дальше страницах — это
 * единственный внутренний путь к части материалов справочника, и закрыть
 * такую страницу целиком (`noindex, nofollow`) значит отрезать их от обхода.
 */
export const withPaginationRobots = (metadata: Metadata): Metadata => ({
  ...metadata,
  robots: { index: false, follow: true }
});

/**
 * Страница-дубль, ожидающая склейки.
 *
 * Отличается от `withPaginationRobots` только поводом, но повод стоит
 * различать: пагинация закрыта навсегда, а дубль — до тех пор, пока для
 * него не зальётся 301 (`prisma/data/redirects.csv`). После заливки такую
 * страницу надо не «открывать обратно», а удалять вместе с маршрутом.
 */
export const withDuplicateRobots = (metadata: Metadata): Metadata => ({
  ...metadata,
  robots: { index: false, follow: true }
});
